	...
	pkgJson.d.results.each{
		...
		json.d.results.each{
			if (artifacts.containsKey(it.Name)){
				artifacts[it.Name].VersionDesigntime << ["${i}":it.Version]
				artifacts[it.Name].VersionRuntime << ["${i}":'']
				artifacts[it.Name].DesigntimePackage << ["${i}":packageDisplayname]
			} else {
				artifacts[it.Name]= [
					Id:it.Name,
					Name:it.DisplayName,
...
private addRuntimeArtifacts(def system, def artifacts, def i){
    ...
    if (artifacts.containsKey(it.Id)){
